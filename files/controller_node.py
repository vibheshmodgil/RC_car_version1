...
RUNTIME TUNING: every gain/tolerance below is a live parameter. The dashboard
(or `ros2 param set /controller_node kp_heading 0.05`) updates them on the fly
via the on-set-parameters callback — no restart needed.
"""
...
class ControllerNode(Node):
    def __init__(self):
        ...
        # live parameter tuning (dashboard / `ros2 param set`)
    ...
    def _on_set_params(self, params):
        for p in params:
            try:
                v = p.value
                if p.name == 'kp_heading':
                    self.heading_pid.kp = float(v)
                elif p.name == 'ki_heading':
                    self.heading_pid.ki = float(v)
                    self.heading_pid._i = 0.0          # reset integrator on Ki change
                elif p.name == 'kd_heading':
                    self.heading_pid.kd = float(v)
                elif p.name == 'max_turn_rate':
                    self.max_turn_rate = float(v)
                    self.heading_pid.out_min = -self.max_turn_rate
                    self.heading_pid.out_max = self.max_turn_rate
                elif p.name == 'cruise_speed':
                    self.cruise = float(v)
                elif p.name == 'heading_tol_deg':
                    self.head_tol = float(v)
                elif p.name == 'distance_tol_m':
                    self.dist_tol = float(v)
                elif p.name == 'nominal_speed_mps':
                    self.nominal_speed = float(v)
                elif p.name == 'distance_source':
                    self.dist_source = str(v)
                elif p.name == 'settle_ticks':
                    self.settle_ticks = int(v)
                elif p.name == 'move_timeout_s':
                    self.move_timeout = float(v)
